...
export default function GenreMultiSelect({onSelectedGenresChange}) {
    ...
    useEffect(() => {
        if (onSelectedGenresChange) {
            onSelectedGenresChange(selectedPeople);
        }
    }, [selectedPeople]);

    const filteredPeople = query === "" ? people : people.filter((person) => person.category_title
        .toLowerCase()
        .replace(/\s+/g, "")
                    ...
                    <div className="relative mt-1">
                        <div
                            ...
                            <Combobox.Input
                                ...
                                displayValue={(people) => people.map((person) => person.category_title).join(", ")}
                            />
                            ...
                            <Combobox.Options
                                ...
                                {filteredPeople.length === 0 && query !== "" ? (
                                    <div className="relative cursor-default select-none py-2 px-4 text-gray-700">
                                        Nothing found.
                                    </div>) : (filteredPeople.map((person) => (<Combobox.Option
                                    key={person.id}
