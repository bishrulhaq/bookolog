...
import {fetchInteraction} from "@/utils";
...
const Comment = ({bookId}) => {
    ...
    const handleLike = async () => {
        ...
        try {
            const response = await fetchInteraction(user_id, bookId, interactionType, status)
    ...
    return (<>
        <div className='mb-2'>
            <div className="flex justify-start dark:bg-gray-600 bg-gray-100 rounded-full py-4">
                ...
                <div className="flex justify-end w-full pr-5">
                    ...
                    <span
                        onClick={handleLike}
                        className={`transition ease-out duration-300 hover:bg-gray-50 bg-gray-100 h-8 px-2 py-2 text-center rounded-full cursor-pointer ${liked ? 'text-red-500' : 'text-black-100'}`}>
                            <svg
                                className="h-4 w-4"
                                fill="none"
                                viewBox="0 0 24 24"
            ...
            <div className="flex w-full">
                <div className="mt-3 mx-5 flex flex-row text-xs">
                    ...
                    <div
                        className="flex text-gray-700 font-normal rounded-md mb-2 mr-4 items-center dark:text-white">Views: <div
                        className="ml-1 text-gray-400 text-ms"> 60k</div>
                    </div>
                </div>
        ...
        {session ? (<div className="bg-white shadow rounded-2xl mb-2 p-4">
        ...
        <div
            className="bg-white border-b-2 border-r-2 border-gray-200 rounded-2xl sm:shadow-sm mx-auto sm:px-4 sm:py-4 md:px-4">
            {comments && comments.map((comment) => (<div
                key={comment.id}
                class="flex-col w-full py-4 px-4">
                <div class="flex flex-row">
                    <Image class="object-cover w-12 h-12 border-2 border-gray-300 rounded-full"
                    ...
                    <div class="flex-col mt-1">
                        <div
                            class="flex items-center flex-1 px-4 font-bold leading-tight">{comment?.user?.user_name}
                            <span
                                class="ml-2 text-xs font-normal text-gray-500"> {formatDistanceToNow(new Date(comment.createdAt), {addSuffix: true})}</span>
                        </div>
                ...
                {replyingTo === comment.id && (<form onSubmit={(e) => handleReplySubmit(e, comment.id)}>
                                    <textarea
                                        value={replyText}
                ...
                {comment?.replies && comment.replies.map((reply) => (<div class="flex flex-row pt-1 md-10 md:ml-16">
                    <Image class="w-12 h-12 border-2 border-gray-300 rounded-full" alt="Emily's avatar"
                    ...
                    <div class="flex-col mt-1">
                        <div
                            class="flex items-center flex-1 px-4 font-bold leading-tight">{reply?.user?.user_name}
                            <span
                                class="ml-2 text-xs font-normal text-gray-500">  {formatDistanceToNow(new Date(reply.createdAt), {addSuffix: true})}</span>
                        </div>
                        <div class="flex-1 px-2 ml-2 text-sm font-medium leading-loose text-gray-600">
                            {reply.content}
                        </div>
