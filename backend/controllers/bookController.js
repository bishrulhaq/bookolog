...
const bookController = {
    getAll: async (req, res) => {
        try {
            ...
            const {count, rows: books} = await book.findAndCountAll({
                order: [['updatedAt', 'DESC']], limit: parseInt(limit), offset: parseInt(offset),
            });
    ...
    featuredBooks: async (req, res) => {
        try {
            const featured = await book.findAll({
                where: {is_featured: true}, order: [['updatedAt', 'DESC']], limit: 8,
            });
    ...
    getById: async (req, res) => {
        ...
        try {
            ...
            if (!response) {
                ...
                const isbn = getBook.data?.volumeInfo?.industryIdentifiers?.find((identifier) => identifier?.type === 'ISBN_13' || identifier?.type === 'ISBN_10')?.identifier;
    ...
    getBookByIdForListing: async (req, res) => {
        const {id} = req.params;
            ...
            if (!response) {
                ...
                if (isbn) {
                    ...
                    await res.status(200).json({
                        status: 200, message: "Book Fetched Successful", data: response
                    });
                } else {
                    res.status(500).json({
    ...
    searchByCategory: async (req, res) => {
        ...
        try {
            const books = await book.findAll({
                where: Sequelize.where(Sequelize.fn('LOWER', Sequelize.col('categories')), 'LIKE', `%${category.toLowerCase()}%`),
                limit: 16,
    ...
    searchByAuthors: async (req, res) => {
        ...
        try {
            const books = await book.findAll({
                where: Sequelize.where(Sequelize.fn('LOWER', Sequelize.col('book_authors')), 'LIKE', `%${author.toLowerCase()}%`),
                limit: 6,
    ...
    searchBooks: async (req, res) => {
        ...
        try {
            const books = await book.findAll({
                where: {
                    [Sequelize.Op.or]: [{title: {[Sequelize.Op.like]: `%${search}%`}}, {isbn_10: {[Sequelize.Op.like]: `%${search}%`}}, {isbn_13: {[Sequelize.Op.like]: `%${search}%`}}, {categories: {[Sequelize.Op.like]: `%${search}%`}}, Sequelize.where(Sequelize.fn('LOWER', Sequelize.col('book_authors')), 'LIKE', `%${search.toLowerCase()}%`)],
                }
            });
            res.json(books);
        } catch (error) {
    ...
    try {
        ...
        if (!existingCategory) {
            await category.create({
                category_title: categoryTitle, background_color: Math.floor(Math.random() * 24),
            }, {transaction: t});
        }
...
function createAuthorData(bookAuthorsArray, authors) {
    ...
    for (const author of authors) {
        ...
        if (authorName) {
            authorData.push({
                name: authorName, key: authorKey || null, k_id: null,
            });
        }
    }
    ...
    bookAuthorsArray.forEach((authorName) => {
        if (!authorData.some((author) => author.name === authorName)) {
            authorData.push({
                name: authorName, key: null, k_id: null,
            });
        }
    });
    ...
    for (const author of author_ids) {
        if (author.key !== null) {
            ...
            if (authorDetails?.data && authorDetails?.data?.name) {
                ...
                alteredAuthors.push({
                    name: addedAuthor?.name, key: addedAuthor?.author_uid, k_id: addedAuthor?.id,
                });
            } else {
                alteredAuthors.push({
                    name: author.name, key: null, k_id: null,
                });
            }
        } else {
            alteredAuthors.push({
...
async function insertAuthorIfNotExists(authorInfo, authorKey, currentBook) {
    try {
        const [authorRecord, created] = await author.findOrCreate({
            where: {author_uid: authorKey}, defaults: {
